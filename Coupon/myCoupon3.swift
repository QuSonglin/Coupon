...
class myCoupon3: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func getStoreList(){
        //(1）设置请求路径
        let url:NSURL = NSURL(string: URLString + "/post_getOwnList")!//不需要传递参数
        ...
        let dataTask=session.dataTask(with: request as URLRequest) { (data, reponse, error) -> Void in
            if (error != nil){
                ...
                print(json)
                //解析字段内的数组
                let storeList = json["storeList"].arrayValue
                
                if (storeList.description != "") {
