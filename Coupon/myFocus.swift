...
class myFocus: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    //func loadDataSource(){
      //  mainArray=NSMutableArray.init(array:["0","1","2","3","4","5","6","7"])
    ...
    func getMyFocus(){
        //(1）设置请求路径
        let url:NSURL = NSURL(string: URLString + "/post_getLikeList")!//不需要传递参数
        ...
        let dataTask=session.dataTask(with: request as URLRequest) { (data, reponse, error) -> Void in
            if (error != nil){
                ...
                print(json)
                //解析字段内的数组
                let likeList = json["likeList"].arrayValue
                
                if (likeList.description != "") {
